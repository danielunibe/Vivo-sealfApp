    ...
    >
      ...
      >
        {periodSummaries.map((period, index) => (
          <div
            key={period.id}
            className={`relative px-5 py-4 ${index > 0 ? 'border-t border-[var(--glass-border)]' : ''}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1 flex flex-col gap-1.5">
                ...
                <div className="flex items-end justify-between gap-2">
                  <p className="text-[1.05rem] font-black leading-none text-[var(--neo-text)]">
                ...
                {(period.currentPieces > 0 || period.moneyGoal > 0) && (
                  <p className="truncate text-[0.5rem] font-semibold leading-snug text-[var(--neo-muted)]">
                ...
                <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-black/5 dark:bg-white/10">
                  <motion.div
                    initial={{ width: 0 }}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-2.5">
        <div className="vivo-surface-on-pattern rounded-[1.2rem] p-3.5 flex flex-col gap-1.5">
          ...
          <p className="text-[1.12rem] font-black leading-none text-[var(--neo-text)]">{remainingAmount > 0 ? formatCurrency(remainingAmount) : 'Cumplido'}</p>
        ...
        <div className="vivo-surface-on-pattern rounded-[1.2rem] p-3.5 flex flex-col gap-1.5">
          ...
          <p className="text-[1.12rem] font-black leading-none text-[var(--neo-text)]">{paceLabel}</p>
      ...
      <motion.div variants={itemVariants} className="flex flex-col gap-2">
        ...
        {sales.length === 0 ? (
          <div className="min-h-[38px] rounded-full border border-dashed border-[var(--glass-border)] vivo-inset-on-pattern px-3 py-2 flex items-center text-[0.68rem] font-bold text-[var(--neo-muted)]">
